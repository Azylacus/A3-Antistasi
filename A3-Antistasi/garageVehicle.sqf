...
if ({isPlayer _x} count crew _veh > 0) exitWith {hint "In order to a store vehicle, its crew must disembark."};

_tipoVeh = typeOf _veh;

if (_veh isKindOf "Man") exitWith {hint "Are you kidding?"};

if (not(_veh isKindOf "AllVehicles")) exitWith {hint "The vehicle you are looking at cannot be added to our Garage"};
...
if (_exit) exitWith {hint "You are the not owner of this vehicle therefore you cannot garage it"};
...
if (_exit) exitWith {hint format ["You cannot garage an air vehicle while near an Airport which belongs to %1",nameBuenos]};
